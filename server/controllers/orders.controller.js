...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    for (const item of ListProduct) {
      ...
      await order.save();
      await Products.findById(item._id).updateOne({
        $inc: { Quantity: -item.Flag },
      });
    }
...
const getCustomerOrderHistory = async (req, res) => {
  ...
  try {
    const orders = await Orders.find({ Customer: CustomerId }).populate(
      "Customer OrderDetails User"
    );
    // populate Product for each OrderDetail
    for (const order of orders) {
...
const getFiveOrders = async (req, res) => {
  ...
  try {
    ...
    if (user.data.Role === "admin") {
      ...
      orders = await Orders.find({ User: user.data.id })
        .populate("User")
        .sort({ createdAt: -1 })
        .limit(5);
    }
